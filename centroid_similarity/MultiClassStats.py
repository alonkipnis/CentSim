...
class MultiClassStats(object):
    ...
    def add_sample(self, X: np.ndarray, y)->None:
        ...
        if y not in self.num.keys(): 
            # we handle the first sample in a class differently
            self.num[y] = 1
            self.sum[y] = X
            self.sum_squares[y] = np.zeros_like(X)
        else:
            # we update class sum and sum-of-squares
            n = self.num[y]
    ...
    def merge_classes(self, ys_to_merge, new_y)->int:
        ...
        for y in ys_to_merge[1:]:
            ...
            self.sum_squares[new_y] += self.sum_squares.pop(y) # this is an approximation
    ...
    def remove_class(self, y):
        self.sum_squares.pop(y)
        self.num.pop(y)
        self.sum.pop(y)
